...
public class HealthBehaviour : MonoBehaviour
{
    ...
    public void Damage(float damage, ModuleType source)
    {
        ...
        text.Set(damage, GameManager._Instance.GetModuleColor(source),
    ...
    {
        PopupText text = ObjectPooler.popupTextPool.Get();
        text.Set(damage, popUpTextColor, transform.position + Vector3.up * (transform.position.y + transform.localScale.y / 2));
        Damage(damage, false);
    }
    ...
    public virtual void Damage(float damage, bool spawnText)
    {
        ...
        if (!spawnText) return;
        PopupText text = ObjectPooler.popupTextPool.Get();
        text.Set(damage, Color.white, transform.position + Vector3.up * (transform.position.y + transform.localScale.y / 2));
    }
    ...
    public virtual void Heal(float healAmount, bool spawnText)
    {
        ...
        if (!spawnText) return;
        PopupText text = ObjectPooler.popupTextPool.Get();
        text.Set("+", healAmount, Color.green, transform.position + Vector3.up * (transform.position.y + transform.localScale.y / 2));
    }
